...
namespace PolishNgramSpellChecker.Tests.Modules
{
    internal static class MisspellsGenerationModule
    {
        ...
        private static Sentence[] GenerateMisspelledText(List<string[]> oryginalSentences, MistakeType mistake)
        {
            ...
            Func<string[], string, Fuzziness, bool, bool, Sentence> func2 = (x, b, c, d, e) => throw new NotImplementedException();
            string method = "w";
            ...
            if (mistake == MistakeType.Clean || mistake == MistakeType.NoDia || mistake == MistakeType.NoDiaAndOrt)
                for (int i = 0; i < n; ++i)
                {
            ...
            else
            {
                for (int i = 0; i < n; ++i)
                {                  
                    results[i] = func2(oryginalSentences[i], method, fuzziness, removeDia, bazMis); // GenerateMisspells(oryginalSentences[i], method, fuzziness);
                    Console.Write("\r{0}%", (i + 1) * 100 / n);
                }
            }
            Console.WriteLine();
        ...
        private static Sentence GenerateMisspells(string[] words, string method, Fuzziness fuzziness, bool removeDia, bool useBaseForm)
        {
